...
for\marginpar{Surely this is too strong. The member has to have some intelligence.}
...
\noindent
We use\marginpar{Dirk, please check!}
$\mathcal{C}$ 
...
Suppose $ba \in\mathcal{B}$, and $bl , h, e, w \in$\marginpar{Surely,
  $ba\subseteq \mathcal{B}$?}
...
\begin{small}\textsf{Measure} (\textsf{initial}($ba$)) = (1,0,0,0,0)\end{small}. Suppose $bl$ $=$ $(l_{1},l_{2})$, for some lists $l_{1}$ and $l_{2}$, and for a given candidate $c$, \textsf{flat} (p c) $=$ $l_{c}$. Then\marginpar{What is flat p c ?}
\begin{small}
\begin{center}
 \textsf{Measure} (\textsf{state} ($ba,t,p,bl,e,h$)) = (0,
 \textsf{length} $h$, $\sum_{d\in l_{2}}^{}$ \textsf{length}
 $l_{c}$,\textsf{length} $l_{1}$,\textsf{length}  $ba$).\marginpar{The
 $\Sigma$ does not make any sense.}
\end{center}
\end{small}
\end{definition}
...
ends''. We have seen that each rule comes with specific\marginpar{This
passage seems garbled to me.}
...
Here, this \textred{What is ``this''?} is captured by the various applicability conditions that\marginpar{Repetition.}
...
permutation of {\fontfamily{qcr}\selectfont ne}. The choice
for\marginpar{I could not understand ``choice for permutation''?}
...
STV~\cite{Assembly:1994:AEP} which stands out for
...
replaced by a different proposition and reads \texttt{nba = last
(p c)}. 
...
programs for each module. The automatic extraction  method provides a
sufficient degree of  verification that the executable behaves in
accordance to its Coq formalisation.\marginpar{This is patently
  false!} Correctness proofs established in the Coq therefore extend
to the executables. 
...
\noindent
Theorem~\ref{termination} guarantees that for each run of the
program\marginpar{Lots of repetition here.}
a \textred{?formal?} certificate exists. Moreover, the theorem
guarantees that the \textred{formal} certificate is the sequence of
computation  performed in the execution to obtain the final result. To
produce a concrete certificate, upon each execution of extracted
Haskell programmes, \marginpar{Delete red words?} we visualise this \textred{formal} sequence of machine states and transitions \textred{which we have encountered}  from the input to reach to the end result.

The certificate generated for each input proves \textgreen{?how?} \marginpar{Where is
  the theorem that says correct certificate implies correct result?} the correctness of the
count to the general public, without having to trust the programmer, 
their own understanding of legal texts, the extraction means used, nor
...
Checkability of the \textred{?visualised?} certificate by \emph{any
  scrutineers} witnesses that our tallying technique satisfies the
count-as-recorded subproperty of universal verifiability. Thus any
election protocol designed for STV schemes which requires a proof of tallying correctness can utilize our tool.
 ...
 elsewhere~\cite{DBLP:conf/voteid/GoreL16}.
 ...
 held in 2006\marginpar{Why does
   the figure say 2008 and 2012?}  and 2008 (Figure~\ref{ref;figure6}). The Molonglo
...
and\marginpar{This requires significant improvements as it rambles.}
...
made prior to the \textred{implementation???}. In the architecture, we have been
...
discharge\marginpar{What does  discharge mean here?} 
...
avoid\marginpar{duplication twice?}
...
could be closed by re-encoding the ML program into the CakeML verified
compiler~\cite{cakeml} to produce a fully verified ML program, but has
not been done to date.

Pattinson and Sh\"urmann~\cite{DBLP:conf/ausai/PattinsonS15}, and
...
idependently verified. \textred{Do they prove that a correct
  certificate implies a correct result?}

Pattinson and Tiwari~\cite{DBLP:conf/itp/PattinsonT17} extend this
method to tackle the Schultz method~\cite{schulze-method}.  Their extracted code handles
